...
AS $function$
  ...
  acum_actual AS (
    ...
    FROM sales_documents
    WHERE location_id = p_location_id
      AND fecha BETWEEN (SELECT mes_inicio FROM mes_info) AND (SELECT ref_date FROM mes_info)
  ...
  mes_anterior AS (
    ...
    FROM sales_documents
    WHERE location_id = p_location_id
      AND fecha BETWEEN (SELECT mes_ant_inicio FROM mes_info) AND (SELECT mes_ant_fin FROM mes_info)
      AND total IS NOT NULL
  ),
  ...
  mes_ant_al_mismo_dia AS (
    ...
    WHERE d.location_id = p_location_id
      AND d.fecha BETWEEN mi.mes_ant_inicio
  ...
  pesos_dia AS (
    ...
    FROM sales_documents
    WHERE location_id = p_location_id
      AND fecha BETWEEN (SELECT ref_date FROM params) - 89 AND (SELECT ref_date FROM params)
      AND total IS NOT NULL
    GROUP BY EXTRACT(isodow FROM fecha)::int
  ),
