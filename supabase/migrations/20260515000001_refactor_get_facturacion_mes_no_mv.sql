-- RPC: get_facturacion_mes (no-MV refactor)
-- Refactored: 2026-05-15. Replaces daily_sales_summary with direct sales_documents aggregation.
-- Preserves fecha column (fecha_caja migration is a separate step).
-- Equivalence: mes_inicio = X  ↔  fecha >= X AND fecha < X + INTERVAL '1 month'
...
BEGIN
  ...
  WHERE location_id = p_location_id
    AND tipo_zona IS NOT NULL
    AND fecha >= v_ante_prev_month_start
    AND fecha < v_prev_month_start;
  ...
  IF v_has_ya_data THEN
    ...
    WHERE location_id = p_location_id
      AND tipo_zona IS NOT NULL
      AND fecha >= v_ya_month_start
      AND fecha < (v_ya_month_start + INTERVAL '1 month')::date;
  END IF;
