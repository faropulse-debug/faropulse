-- RPC: get_facturacion_semana — fecha → fecha_caja
-- Migration: 2026-05-15. Builds on no-MV refactor (000002); switches fecha to fecha_caja.
...
BEGIN
  ...
  SELECT
    ...
    COUNT(DISTINCT fecha_caja)
  INTO v_fact_actual, v_dias_actual
  FROM sales_documents
  WHERE location_id = p_location_id
    AND tipo_zona IS NOT NULL
    AND fecha_caja BETWEEN v_week_start AND v_week_end;
  ...
  WHERE location_id = p_location_id
    AND tipo_zona IS NOT NULL
    AND fecha_caja BETWEEN v_comp_week_start AND v_comp_week_end;
  ...
  IF v_has_ya_data THEN
    ...
    WHERE location_id = p_location_id
      AND tipo_zona IS NOT NULL
      AND fecha_caja BETWEEN v_ya_week_start AND v_ya_week_start + 6;
  END IF;

  -- Rolling 28 días actual
  SELECT COALESCE(SUM(total), 0)
  INTO v_rolling
  FROM sales_documents
  WHERE location_id = p_location_id
    AND tipo_zona IS NOT NULL
    AND fecha_caja BETWEEN v_roll_start AND v_roll_end;

  -- Rolling 28 días anterior
  SELECT COALESCE(SUM(total), 0)
  INTO v_rolling_prev
  FROM sales_documents
  WHERE location_id = p_location_id
    AND tipo_zona IS NOT NULL
    AND fecha_caja BETWEEN v_prev_roll_start AND v_prev_roll_end;
